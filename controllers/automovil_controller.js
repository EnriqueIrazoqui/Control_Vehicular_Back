...
const buscar_automovil = async (req, res) => {
    ...
    await pool.getConnection().then( async (conn) => {
        try{
            ...
            const query = await conn.query("SELECT FROM Vehiculo WHERE idVehiculo = ?", idVehiculo);
            console.log(query);
            ...
            res.status(200).json({
                ...
                'mensaje': {
                    'statusCode': 200,
                    'menssageText':'El vehiculo fue encontrado con exito'
                }
            })
            conn.end();
        }catch(error){
